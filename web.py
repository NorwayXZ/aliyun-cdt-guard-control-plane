...
REGION_NAME_MAP = dict(ALIYUN_REGION_OPTIONS)


def region_display_text(region_id: str | None) -> str:
    region_id = str(region_id or "").strip()
    if not region_id:
        return "区域未知"
    label = REGION_NAME_MAP.get(region_id)
    if label:
        return f"{label} · {region_id}"
    return region_id
...
def flash_message(code: str) -> str:
    messages = {
        ...
        "user_notify_saved": "我的通知设置已保存",
        "user_notify_test_sent": "已发送个人测试通知，请检查 Telegram",
        "user_notify_test_failed": "个人测试通知发送失败，请检查 Bot Token 和 Chat ID",
...
def page_shell(
    ...
    if is_admin(user):
        ...
        config_nav = [
            ("/notifications", "notifications", "我的通知", "◉"),
            ("/security", "security", "账号安全", "◇"),
        ]
...
<head>
  ...
  <style>
    ...
    .server-metric-main span,
    .server-metric-side span {{
      color: var(--muted);
      font-size: 12px;
      font-weight: 650;
      line-height: 1.25;
    ...
    function initAssetBoard() {{
      ...
      board.querySelectorAll("[data-copy-ip]").forEach((button) => {{
        button.addEventListener("click", async (event) => {{
          ...
          const text = button.dataset.copyIp || "";
...
def render_server_row(item: dict, metadata: dict[str, dict], history: list[dict], active: bool = False, user: dict | None = None) -> str:
    ...
    status_html = status_dot_html(state_class, state_label)
    region_line = region_display_text(item.get("region_id"))
    ip_value = identity["primary_ip"]
...
def render_user_notifications_page(query: dict[str, list[str]] | None = None, user: dict | None = None) -> bytes:
    query = query or {}
    ...
    """
    return page_shell(
        "notifications", "我的通知", "配置只属于当前账号的告警接收渠道", body,
        actions='<a href="/" class="btn">返回主页</a>', flash=query.get("flash", [""])[0], auto_refresh=False, user=user,
    )


def save_user_notifications(fields: dict[str, list[str]], user: dict) -> tuple[bool, str]:
    username = str(user.get("username") or "")
    existing = notifications.load_user_notification_config(username)
    telegram = existing.setdefault("telegram", {})
    token = form_value(fields, "telegram_bot_token")
    chat_id = form_value(fields, "telegram_chat_id")
    if chat_id and not valid_telegram_chat_id(chat_id):
        return False, "telegram_chat_invalid"
    if token:
        telegram["bot_token"] = token
    if chat_id:
        telegram["chat_id"] = chat_id
    existing["enabled"] = checked(fields, "enabled")
    existing["telegram"] = {
        **telegram,
    ...
    notifications.save_user_notification_config(username, existing)
    return True, "saved"


def send_user_notification_test(user: dict) -> dict:
    username = str(user.get("username") or "")
    config = notifications.load_user_notification_config(username)
...
def send_user_power_notification(user: dict, item: dict, action: str) -> None:
    config = notifications.load_user_notification_config(str(user.get("username") or ""))
    if not config.get("enabled") or not config.get("rules", {}).get("notify_actions"):
        return
    label = first_value(item.get("product_name"), item.get("label"), item.get("instance_id"), default="服务器")
    action_text = "手动开机" if action == "start" else "手动关机"
    notifications.send_message(
        f"Aliyun CDT Guard {action_text}",
        f"服务器：{label}\n操作：{action_text}\n这条消息只发送给当前账号。",
        {"type": "user_manual_power", "server_id": server_id_of(item), "username": user.get("username")},
        config,
    )
      ...
      <aside class="card">
        ...
        <div class="card-body">
          <form method="post" action="/eip/save" data-save-form>
            ...
            {checkbox_field("eip_auto_adjust_enabled", "自动申请目标带宽", bool(settings.get("auto_adjust_enabled")), "按流量计费 EIP 会每 24 小时尝试一次；阿里云一旦允许会立即调整并发送 Telegram 通知。已绑定共享带宽包的 EIP 也会尝试，但不会自动解除共享关系。")}
...
def render_user_security_page(query: dict[str, list[str]] | None = None, user: dict | None = None) -> bytes:
    query = query or {}
    username = str((user or {}).get("username") or "")
    body = f"""
      <div class="form-layout">
        <form class="card save-form" method="post" action="/security/user/save" data-save-form>
          ...
          <div class="card-body">
            ...
            <section class="form-section">
              <div class="credential-grid">
                {input_field("new_password", "新密码", "", "password", placeholder="至少 8 位", required=True)}
                {input_field("confirm_password", "确认新密码", "", "password", placeholder="再次输入新密码", required=True)}
              </div>
            </section>
          </div>
          <div class="card-footer d-flex align-items-center gap-2"><button class="btn btn-primary ms-auto" type="submit" data-submit-button data-loading-text="正在保存...">保存新密码</button></div>
        </form>
        <aside class="card guide-panel"><div class="card-header"><h3 class="card-title">账号安全</h3></div><div class="card-body"><div class="guide-step"><strong>当前账号</strong><span>{esc(username)}</span></div><div class="guide-step"><strong>独立会话</strong><span>修改密码后，当前账号的旧会话会自动失效。</span></div></div></aside>
      </div>
    """
    return page_shell("security", "账号安全", "修改当前登录账号的密码", body, actions='<a href="/" class="btn">返回主页</a>', flash=query.get("flash", [""])[0], auto_refresh=False, user=user)


def save_user_security_settings(fields: dict[str, list[str]], user: dict) -> tuple[bool, str]:
    username = str(user.get("username") or "")
    if form_value(fields, "username") != username:
        return False, "username_empty"
    password = form_value(fields, "new_password")
    confirm = form_value(fields, "confirm_password")
    if password != confirm:
        return False, "password_mismatch"
    if len(password) < 8:
        return False, "password_short"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/notifications":
            if is_admin(user):
                ...
                self.send_bytes(render_user_notifications_page(query, user=user), "text/html; charset=utf-8")
            return
        if parsed.path == "/eip":
            if not is_admin(user):
        ...
        if parsed.path == "/security":
            if is_admin(user):
                ...
                self.send_bytes(render_user_security_page(query, user=user), "text/html; charset=utf-8")
            return
    ...
    def do_POST(self):
        ...
        if parsed.path == "/servers/power":
            ...
            if ok and not is_admin(user):
                try:
                    send_user_power_notification(user, item, power_action)
                except Exception:
                    pass
        ...
        if parsed.path == "/notifications/save":
            ...
            return
        if parsed.path == "/notifications/user/save":
            if is_admin(user):
                self.send_access_denied()
                return
            ok, reason = save_user_notifications(fields, user)
            self.redirect("/notifications?flash=user_notify_saved" if ok else f"/notifications?flash={reason}")
            return
        if parsed.path == "/notifications/user/test":
            if is_admin(user):
                self.send_access_denied()
                return
            result = send_user_notification_test(user)
            self.redirect("/notifications?flash=user_notify_test_sent" if result.get("ok") else "/notifications?flash=user_notify_test_failed")
            return
        ...
        if parsed.path == "/security/save":
            ...
            return
        if parsed.path == "/security/user/save":
            if is_admin(user):
                self.send_access_denied()
                return
            ok, reason = save_user_security_settings(fields, user)
            if ok:
                self.send_response(HTTPStatus.SEE_OTHER)
                self.send_header("Location", "/login?flash=security_saved")
                self.send_header("Set-Cookie", clear_session_cookie())
